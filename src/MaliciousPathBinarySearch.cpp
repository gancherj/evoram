...
{
   std::unique_ptr<PathBlock> blockptr = std::move(client.Read(blockid));
   int out;
...
int main(int argc, char** argv)
{
   ...
   size_t num_real_blocks = ((1 << height) - 1) * blocksperbucket;
   ...
   bandwidth<<height<<", "<<blocksperbucket<<", "<<virtual_bandwidth<<std::endl;
   bandwidth.close();
}
